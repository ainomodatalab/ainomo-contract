...
contract ReportUtilsMock {
    ...
    function encode(uint64 Balance, uint32 Validators) public pure returns (uint256) {
        return ReportUtils.encode(Balance, Validators);
    }

    function decode(uint256 value) public pure returns (uint64 Balance, uint32 Validators) {
        return value.decode();
    }
