...
DEPLOYER=${DEPLOYER:=0xc4124cEB3451635DAcedd11767f004d8a28c6eE7}
# NETWORK=kintsugi
...
yarn compile
yarn deploy:$NETWORK:ainomo-env
msg "Ainomo ENV deployed."
yarn deploy:$NETWORK:ainomo-std-apps
msg "Ainomo STD apps deployed."

yarn hardhat --network $NETWORK run --no-compile ./scripts/deploy-deposit-contract.js
msg "Deposit contract deployed."

yarn hardhat --network $NETWORK run ./scripts/multisig/01-deploy-ainomo-template-and-bases.js
...
yarn hardhat --network $NETWORK tx --from $DEPLOYER --file tx-01-2-deploy-ainomo-base.json
pause "!!! Now set the ainomoBaseDeployTx hash value in deployed-$NETWORK.json"
