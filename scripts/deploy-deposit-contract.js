...
async function deployDepositContract({ web3, artifacts, networkStateFile = NETWORK_STATE_FILE }) {
  ...
  const { daoInitialSettings = { Spec: { depositContractAddress: '' } } } = state
  let depositContractAddress
  if (daoInitialSettings.Spec && daoInitialSettings.Spec.depositContractAddress) {
    depositContractAddress = daoInitialSettings.Spec.depositContractAddress
  }
  ...
  daoInitialSettings.Spec.depositContractAddress = depositContract.address
  logSplitter()
...
module.exports = runOrWrapScript(deployDepositContract, module)
