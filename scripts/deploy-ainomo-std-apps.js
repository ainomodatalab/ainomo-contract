...
const { readAppName } = require('./helpers/ainomo')
const { gitCloneRepo } = require('./helpers/git')

const { uploadDirToIpfs } = require('../buidler-ainomo/dist/src/utils/ipfs')
const { toContentUri } = require('../buidler-ainomo/dist/src/utils/apm/utils')

const APPS = ['agent', 'finance', 'token-manager', 'vault', 'voting']

const NETWORK_STATE_FILE = process.env.NETWORK_STATE_FILE || 'deployed.json'
const AINOMO_APPS_REPO_REF = process.env.AINOMO_APPS_REPO_REF || 'master'
...
async function deployAinomoStdApps({
  web3,
  artifacts,
  networkStateFile = NETWORK_STATE_FILE,
  ainomoAppsRepo = AINOMO_APPS_REPO,
  ainomoAppsRepoRef = AINOMO_APPS_REPO_REF,
  ...
  logHeader(`Checking out ainomo-apps repo...`)
  const appsRepoPath = './ainomo-apps'
  await gitCloneRepo(appsRepoPath, ainomoAppsRepo, ainomoAppsRepoRef)
...
async function publishApp(appName, appsRepoPath, hardhatConfig, env, netName, releaseType, netState) {
  ...
  const childEnv = {
    ...env,
    STD_APPS_DEPLOY: '1',
    APP_FRONTEND_PATH: `ainomo-apps/apps/${appName}/app`,
    APP_FRONTEND_DIST_PATH: `ainomo-apps/apps/${appName}/dist`
  }
...
module.exports = runOrWrapScript(deployAinomoStdApps, module)
